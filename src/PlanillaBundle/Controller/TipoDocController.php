...
class TipoDocController extends Controller {
    ...
    public function indexAction(Request $request) {
        ...
        $tipoDocs = $tipoDoc_repo->findBy([], ['estado' => 'DESC', 'id' => 'ASC']);

        return $this->render("@Planilla/tipoDoc/index.html.twig", ["tipoDocs" => $tipoDocs]);
    }

    public function addAction(Request $request) {
        $tipoDoc = new TipoDoc();
        $em = $this->getDoctrine()->getManager();
        $sth1 = $em->getConnection()->prepare("SELECT SugerirTipoDoc()");
        $sth1->execute();
        ...
        if ($form->isSubmitted()) {
            if ($form->isValid()) {
                $tipoDoc_repo = $em->getRepository("PlanillaBundle:TipoDoc");
                $tipoDoc = $tipoDoc_repo->findOneBy(["nombre" => $form->get("nombre")->getData()]);
                if ($tipoDoc != null) {
                    ...
                    $sth = $em->getConnection()->prepare("CALL AgregarTipoDoc(:id, :nombre, :estado)");
                    $sth->bindValue(':id', $id);
        ...
        return $this->render('@Planilla/tipoDoc/add.html.twig', ["form" => $form->createView()]);
    }
    ...
    public function editAction(Request $request, $id) {
        ...
        return $this->render('@Planilla/tipoDoc/edit.html.twig', ["form" => $form->createView()]);
    }
